...
A single-metric win is rarely a real win. Pair every primary metric with a counter-metric (also called guardrail) that catches the trade-off ([Mixpanel](https://mixpanel.com/blog/guardrail-metrics/), [Amplitude North Star](https://amplitude.com/books/north-star/about-north-star-framework)).
...
Rule of thumb from Mixpanel: 2–3 counter-metrics per experiment. More than that produces noise and false positives.
...
**Pattern 3 — Primary + counter (Mixpanel / Amplitude).**
> Illustrative pairing: "Conversion rose from 58% to 65% (+7 pp, n = 142,000). Refund rate held flat at 2.1%, confirming the lift is durable rather than driven by impulse." Real published cases that follow this primary+counter shape: Mixpanel's [Instagram Stories vs. main feed engagement](https://mixpanel.com/blog/guardrail-metrics/) and [Airbnb bookings vs. user ratings](https://mixpanel.com/blog/guardrail-metrics/) pairings.
