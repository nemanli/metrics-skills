...
description: Use when the user has metric results and asks how to communicate them — quarterly review, stakeholder defence, board slide, design impact story, retro deck, or "how do I explain this to the CPO/CFO/board?". Use when the user wants to translate design findings into business language, frame a counter-metric, or pick the right format for an audience. Skip for picking which metrics to track (use metrics-ux). Skip for figuring out why a number moved (use metrics-diagnose). Skip for raw HEART definitions (use metrics-basics).
allowed-tools:
---
...
- **Wrong skill.** If the user is picking metrics → `metrics-ux`. If diagnosing a movement → `metrics-diagnose`. If asking for HEART definitions → `metrics-basics`.
