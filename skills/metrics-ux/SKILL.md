---
name: metrics-ux
description: Recommends UX metrics (HEART — happiness, engagement, adoption, retention, task success) for a real design situation: redesign, onboarding flow, feature launch, "what should I measure?". Skip if a metric has already moved (use metrics-diagnose), if presenting results to stakeholders (use metrics-present), for pure framework definitions (use metrics-basics), or for monetization metrics (revenue, MRR — out of scope).
allowed-tools: Read
---
...
- The user wants a written measurement plan with a decision rule **before** the change ships → use `metrics-spec`
- The user wants to design the events/tracking plan that captures a metric → use `metrics-instrumentation`
- The user wants to stress-test someone's claim or self-review before publishing → use `metrics-review`
...
- **Wrong skill.** If the user is diagnosing a metric drop ("retention dropped, why?") → redirect to `metrics-diagnose`. If they want a pre-launch measurement plan with a decision rule → `metrics-spec`. If designing the tracking plan → `metrics-instrumentation`. If reviewing someone's claim → `metrics-review`. If presenting results to stakeholders → `metrics-present`.
