...
**Pitfalls.**
- "Action" definition is the whole game. Counting every click, scroll, and hover produces noise; counting only the rare "ideal" action produces near-zero everywhere. Pick a small set of actions that represent product value — industry frameworks ([Amplitude's North Star inputs](https://amplitude.com/blog/product-north-star-metric)) commonly cite three to five as a workable range; Rodden's HEART guidance is "a handful, not everything."
