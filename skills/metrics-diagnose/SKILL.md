...
description: Use when the user has metric data (numbers, CSV, or a described change) and asks why a metric moved, whether the change is real, what to check next, or what the data signals. Use when a metric dropped, spiked, plateaued, or behaves unexpectedly after a release. Use when the user shares before/after numbers, a funnel, or a cohort table and wants interpretation. Skip when no data exists yet (use metrics-ux to plan instrumentation). Skip for stakeholder communication (use metrics-present). Skip for pure metric definitions (use metrics-ux or metrics-basics).
allowed-tools: Read
---
...
Pick the test by metric type. Compute internally; report the result, not the derivation.

| Metric type | Test | Why |
|---|---|---|
| **Proportion** (success rate, retention rate, adoption rate, drop-off, conversion) | Two-proportion z-test | Compares two binomial proportions with known n |
| **Continuous** (time on task, session duration, NPS score, SUS score) | Welch's t-test (unequal variances) | Compares two means without assuming equal variance |
| **Count** (events per user, errors per session) | Chi-square test of independence (categorical) or Welch's t-test on per-user means (when distribution is reasonable) | Depends on whether comparing distributions or means |
| **Rate over time** (funnel step rate week over week) | Two-proportion z-test on the suspect window vs baseline window | Treat each window as an independent sample |

For each test compute:

1. **p-value** — probability the observed change happened by chance under the null hypothesis.
2. **95% confidence interval** — exact CI on the difference (Wilson for proportions; standard for means).
3. **Effect size** — Cohen's h for proportions; Cohen's d for continuous. Interpret: small (h/d ≈ 0.2), medium (≈ 0.5), large (≈ 0.8).
4. **Power check** (if the test is non-significant) — what minimum detectable effect (MDE) the current sample size could detect at 80% power. If the observed change is below MDE, the test is underpowered, not negative.

Report format (one block, no formulas):

> **Signal vs noise:** [Statistically significant / Not significant / Underpowered]. [Two-proportion z-test / Welch's t-test], p=[value], 95% CI on the difference: [lower, upper], effect size [h/d]=[value] ([small/medium/large]). [One-sentence practical interpretation: "Drop is real and large enough to act on" / "Drop is within noise — wait for more data" / "Sample too small to detect a change of this size".]
...
> **Signal vs noise:** Statistically significant. Two-proportion z-test, p<0.001, 95% CI on the difference: −16.5% to −9.5%, Cohen's h=0.28 (small-to-medium). Drop is real and unlikely to be noise. Effect size is small in statistical terms but operationally meaningful at this scale (≈150 fewer successful users per week).
...
> **Signal vs noise:** Not significant — and underpowered. Two-proportion z-test, p=0.30, 95% CI on the difference: −22.6% to +6.6%, Cohen's h=0.16 (small). At n≈80 per group, the minimum detectable effect at 80% power is roughly 16 percentage points; the observed 8-point drop is well below that threshold. Conclusion: the data **cannot tell** whether retention truly dropped, stayed flat, or improved slightly. The wide CI spans both meaningful drop and meaningful gain.
...
- **Wrong skill.** If the user wants to plan what to measure → redirect to `metrics-ux`. If presenting the diagnosis to stakeholders → redirect to `metrics-present`.
