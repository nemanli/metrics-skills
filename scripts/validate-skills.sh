...
validate_skill_md() {
  ...
  issues="$issues$(validate_links "$file")"
...
# Resolves internal (and optionally external) links for a file.
# Echoes accumulated issues (each prefixed with "; "); empty if none.
# Callers capture via $(...) and append to their own issues string.
validate_links() {
  local file="$1"
  local file_dir
  ...
  while IFS= read -r link; do
    ...
    if [ ! -e "$target" ]; then
      errors="$errors; broken internal link: $link"
    fi
  ...
  if [ "$CHECK_EXTERNAL" = "1" ]; then
    while IFS= read -r url; do
      ...
      if ! echo "$code" | grep -qE '^(2|3)[0-9][0-9]$'; then
        errors="$errors; external link $code: $url"
      fi
