# Contributing to UX Metrics Skills

Thanks for your interest in contributing! This is a collection of skills that help UX designers choose, diagnose, and present UX metrics.
...
The `allowed-tools` field lists which Claude tools the skill is permitted to use. `Read` is required for any skill that loads its own reference files or reads files the user shares (CSVs, exports). Leave empty only if the skill never reads from disk.
...
Version is stored in three places — `VERSION`, `.claude-plugin/plugin.json`, and the README badge. Use the helper to keep them in lockstep:
